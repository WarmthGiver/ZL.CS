...
namespace ZL.CS.API
{
    public sealed class Camera : Component
    {
        ...
        private static CharCell[,] foregroundCharMap;
        ...
        protected override void Start()
        {
            ...
            foregroundCharMap = new CharCell[size.Height, size.Width];
        ...
        internal void Clear()
        {
            ...
            if (WillClear == true)
            {
                ...
                foregroundCharMap.Fill(new CharCell(' ', ' '));
            }
        ...
        { 
            ...
            if (graphic is Background background)
            {
                for (int y = graphicRect.Y; y < graphicRect.Height; ++y)
                {
                    ...
                    for (int x = graphicRect.X; x < graphicRect.Width; ++x)
                    {
                        ...
                        foregroundCharMap.Set(mapPoint, new(' ', ' '));
                    }
                }
            }
            ...
            else if (graphic is Foreground foreground)
            {
                for (int y = graphicRect.Y; y < graphicRect.Height; ++y)
                {
                    ...
                    for (int x = graphicRect.X; x < graphicRect.Width && x < foreground.charCellMap[y].Count; ++x)
                    {
                        mapPoint.X = mapLocation.X + x;
                        ...
                        foregroundCharMap.Set(mapPoint, foreground.charCellMap[y][x]);
                    }
                }
            }
            ...
            else if (graphic is Text text)
            {
                ...
                for (int x = graphicRect.X; x < graphicRect.Width; ++x)
                {
                    ...
                    if (text.colorMap != null)
                    {
                        foregroundColorMap.Set(mapPoint, text.colorMap[y, x]);
                    }

                    foregroundCharMap.Set(mapPoint, text.charCellMap[x]);
                }
            }
        }
